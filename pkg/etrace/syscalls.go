...
func (sd SyscallDefinition) MarshalBinary() ([]byte, error) {
	rawSyscall := make([]byte, 6*16)
	for i, arg := range sd.Arguments {
		rawArg, err := arg.MarshalBinary()
		if err != nil {
			return nil, err
		}
		copy(rawSyscall[i*16:], rawArg)
	}
	return rawSyscall, nil
}

type DynamicSizeResolutionType int

const (
	UnknownDynamicResolution DynamicSizeResolutionType = 0
	Arg0                     DynamicSizeResolutionType = 1 << 0
	Arg1                     DynamicSizeResolutionType = 1 << 1
	Arg2                     DynamicSizeResolutionType = 1 << 2
	Arg3                     DynamicSizeResolutionType = 1 << 3
	Arg4                     DynamicSizeResolutionType = 1 << 4
	Arg5                     DynamicSizeResolutionType = 1 << 5
	ReturnValue              DynamicSizeResolutionType = 1 << 6
	TrailingZero             DynamicSizeResolutionType = 1 << 7
)
...
type SyscallArgument struct {
	...
	Size                  int
	SizeMultiplier        int
	DynamicSizeResolution DynamicSizeResolutionType
	TypeName              string
	StarCount             int
...
func (sa *SyscallArgument) MarshalBinary() ([]byte, error) {
	rawArg := make([]byte, 16)
	ByteOrder.PutUint32(rawArg[:4], uint32(sa.DereferenceCount))
	ByteOrder.PutUint32(rawArg[4:8], uint32(sa.Size))
	ByteOrder.PutUint32(rawArg[8:12], uint32(sa.SizeMultiplier))
	ByteOrder.PutUint32(rawArg[12:16], uint32(sa.DynamicSizeResolution))
	return rawArg, nil
}

// ParseType parses the argument type from its definition
func (sa *SyscallArgument) ParseType() {
	if len(sa.TypeName) == 0 {
		sa.TypeName = sa.Definition
		if strings.HasSuffix(sa.TypeName, " *") {
			sa.TypeName = strings.TrimSuffix(sa.TypeName, " *")
			sa.StarCount = 1
			sa.DereferenceCount = 1
		}
		if strings.HasSuffix(sa.TypeName, " *") {
			sa.TypeName = strings.TrimSuffix(sa.TypeName, " *")
			sa.StarCount = 2
			sa.DereferenceCount = 2
		}
		if strings.HasSuffix(sa.TypeName, " *const") {
			sa.TypeName = strings.TrimSuffix(sa.TypeName, " *const")
			sa.StarCount = 2
			sa.DereferenceCount = 2
		}
		if strings.HasPrefix(sa.TypeName, "const ") {
			sa.TypeName = strings.TrimPrefix(sa.TypeName, "const ")
		}
		if strings.HasPrefix(sa.TypeName, "unsigned ") {
			sa.TypeName = strings.TrimPrefix(sa.TypeName, "unsigned ")
		}
		if strings.HasPrefix(sa.TypeName, "struct") {
			sa.TypeName = strings.TrimPrefix(sa.TypeName, "struct ")
			sa.BTFType = new(btf.Struct)
		}
		if strings.HasPrefix(sa.TypeName, "union") {
			sa.TypeName = strings.TrimPrefix(sa.TypeName, "union ")
			sa.BTFType = new(btf.Union)
		}
		if strings.HasSuffix(sa.TypeName, "_t") || strings.HasSuffix(sa.TypeName, "_set") {
			sa.BTFType = new(btf.Typedef)
		}
	}
}

// ResolveSizeFromBTFType resolves the argument size from its BTF type
func (sa *SyscallArgument) ResolveSizeFromBTFType() {
	switch reflect.TypeOf(sa.BTFType) {
	case reflect.TypeOf(&btf.Struct{}):
		sa.Size = int(sa.BTFType.(*btf.Struct).Size)
	case reflect.TypeOf(&btf.Union{}):
		sa.Size = int(sa.BTFType.(*btf.Union).Size)
	case reflect.TypeOf(&btf.Typedef{}):
		// dereference type definition until an actual struct or union is found
		sa.BTFType = sa.BTFType.(*btf.Typedef).Type
		sa.ResolveSizeFromBTFType()
	}
}

// ResolveDynamicSizeResolutionType resolves dynamic parameters of the current argument
func (sa *SyscallArgument) ResolveDynamicSizeResolutionType(nr Syscall, argIndex int, spec *btf.Spec) {
	// set default multiplier
	sa.SizeMultiplier = 1

	switch nr {
	case SysAccess, SysAcct, SysChdir, SysChmod, SysChown, SysChroot, SysCreat, SysDeleteModule, SysExecve, SysExecveat,
		SysFaccessat, SysFaccessat2, SysFanotifyMark, SysFchmodat, SysFchownat, SysFinitModule, SysFremovexattr,
		SysFsopen, SysFspick, SysFutimesat, SysGetcwd, SysInotifyAddWatch, SysKexecFileLoad, SysLchown, SysLink,
		SysLinkat, SysLookupDcookie, SysLremovexattr, SysMemfdCreate, SysMkdir, SysMkdirat, SysMknod, SysMknodat,
		SysMount, SysMoveMount, SysMqOpen, SysMqUnlink, SysNameToHandleAt, SysNewfstatat, SysLstat, SysStat, SysOpen,
		SysOpenTree, SysOpenat, SysOpenat2, SysPivotRoot, SysRemovexattr, SysRename, SysRenameat, SysRenameat2,
		SysRequestKey, SysRmdir, SysSymlink, SysSymlinkat, SysTruncate, SysUmount2, SysUnlink, SysUnlinkat, SysUtime,
		SysUtimensat, SysUtimes, SysStatfs, SysStatx, SysSwapon, SysSwapoff:
		sa.DynamicSizeResolution = TrailingZero
	case SysPread64, SysRead, SysRecvfrom:
		sa.DynamicSizeResolution = ReturnValue
	case SysAddKey:
		if argIndex <= 1 {
			sa.DynamicSizeResolution = TrailingZero
		} else if argIndex == 2 {
			sa.DynamicSizeResolution = Arg3
		}
	case SysFgetxattr:
		if argIndex == 1 {
			sa.DynamicSizeResolution = TrailingZero
	...
	case SysFlistxattr:
		if argIndex == 1 {
			sa.DynamicSizeResolution = Arg2
		}
	case SysFsconfig:
		if argIndex == 2 {
			sa.DynamicSizeResolution = TrailingZero
		} else if argIndex == 3 {
			sa.DynamicSizeResolution = Arg4 | TrailingZero
		}
	case SysFsetxattr:
		if argIndex == 1 {
			sa.DynamicSizeResolution = TrailingZero
	...
	case SysGetrandom:
		if argIndex == 0 {
			sa.DynamicSizeResolution = Arg1
		}
	case SysGetsockopt:
		if argIndex == 3 {
			sa.DynamicSizeResolution = Arg4
		}
	case SysGetxattr:
		if argIndex <= 1 {
			sa.DynamicSizeResolution = TrailingZero
	...
	case SysIoUringEnter:
		if argIndex == 4 {
			sa.DynamicSizeResolution = Arg5
		}
	case SysIoUringRegister:
		if argIndex == 2 {
			// compute size of multiplier
			var iovecType btf.Struct
			if err := spec.FindType("iovec", &iovecType); err != nil {
				sa.DynamicSizeResolution = Arg3
				sa.SizeMultiplier = 16
			} else {
				sa.DynamicSizeResolution = Arg3
				sa.SizeMultiplier = int(iovecType.Size)
			}
		}
	case SysLgetxattr:
		if argIndex <= 1 {
	...
	case SysListxattr, SysLlistxattr:
		if argIndex == 0 {
			sa.DynamicSizeResolution = TrailingZero
		}
		if argIndex == 1 {
	...
	case SysMincore:
		if argIndex == 2 {
			sa.DynamicSizeResolution = Arg1
			sa.SizeMultiplier = 1 / os.Getpagesize()
		}
	case SysModifyLdt:
		if argIndex == 1 {
			sa.DynamicSizeResolution = Arg2
	...
	case SysMqTimedreceive, SysMqTimedsend:
		if argIndex == 1 {
			sa.DynamicSizeResolution = Arg2
		}
	case SysPselect6:
		if argIndex == 5 {
			var sigsetType btf.Typedef
			if err := spec.FindType("sigset_t", &sigsetType); err != nil {
				// default values
				sa.Size = 8
			} else {
				sa.Size = int(sigsetType.Type.(*btf.Struct).Size)
			}
		}
	case SysPwrite64:
		if argIndex == 1 {
	...
	case SysSetdomainname:
		if argIndex == 0 {
			sa.DynamicSizeResolution = Arg1
		}
	default:
		sa.DynamicSizeResolution = UnknownDynamicResolution
		sa.SizeMultiplier = 0
	}
}
